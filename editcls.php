...
{    }
    ...
    header("Location: login.php");
        ...
        if(isset($_GET['edit']))
        {
            ...
            $query = "SELECT * FROM classroom WHERE crsn='".$_SESSION['snm']."'";
            $result = mysqli_query($connect,$query);
            $row = mysqli_fetch_array($result);
            $options = $row['crtype'];
        }
        
        if(isset($_POST['newcrsn']))
        {
            $crname = $_POST['newcrnm'];
            $crcode = $_POST['newcrsn'];
            $cp = $_POST['newcrcp'];
            $crtype = $_POST['newcrtype'];
            
            $queryup = "UPDATE classroom SET crnm='$crname',crsn='$crcode',crcp='$cp',crtype='$crtype' WHERE crsn='".$_SESSION['snm']."'";
            $result2 = mysqli_query($connect,$queryup);
            header("Location:classroomview.php");
        ...
        <div class="container-fluid">
            <div class="col-md-4" style="width:100%;">
                <div class="form_main">
                    ...
                    <div class="form">
                        <form action="editcls.php" method="post" id="contactFrm" name="classroomform">
                            <strong>Enter Name of the classroom:</strong><br><input type="text" required placeholder="Classroom Name" value="<?php echo $row[0];?>" name="newcrnm" class="txt"><br><br>
                            <strong>Enter short name:</strong><br><input type="text" required placeholder="Short code" value="<?php echo $row[1];?>" name="newcrsn" class="txt"><br><br>
                            <strong>Enter Room capacity:</strong><br><input type="number" required placeholder="Capacity" value="<?php echo $row[2];?>" name="newcrcp" class="txt"><br><br>
                            <strong>Select Classroom Type:</strong><br><br>
                            &nbsp;&nbsp;&nbsp;  <select name ="newcrtype">
                                                    <option value="class" <?php if($options=="class") echo 'selected="selected"'; ?>>Classroom</option>
                                                    <option value="lab" <?php if($options=="lab") echo 'selected="selected"'; ?>>Lab</option>
                                                    <option value="tutorial" <?php if($options=="tutorial") echo 'selected="selected"'; ?>>Tutorial Room</option>
                                                </select><br><br>
                            <input style="background-color: darkslategray;color: white" type="submit" class="btn btn-primary" value="Go Back" name="back" onclick="window.location.href='classroomview.php'" class="txt2">

                            <input style="background-color: orangered;color: black" type="submit" class="btn btn-primary" value="Update" name="update" class="txt2">
                        </form>
                    </div>
                </div>
            </div>
